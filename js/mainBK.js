...
//		nodeHUD.append('test');
	...
	function spawnAvatar(x,z){
		var cAvatar = new ME3D.Avatar();
		myWorld.scene.add(cAvatar.getAvatar());
		cAvatar.getAvatar().position.x = x;
		cAvatar.getAvatar().position.z = z;
		//console.log(myBounds.getBoundsMax(cAvatar.getBoundsMesh()));
		var cAvatarBounds = myBounds.getBoundsMax(cAvatar.getBoundsMesh());
		cAvatarBounds.multiplyScalar(2);
		//console.log(avatarBounds);
		var cAvatarController = myPhysics.addMassBody(cAvatar.getAvatar(),cAvatarBounds,3);
	};
	...
	// controls	
	///////////////////////////////////
	var controlsA = new ME3D.AvatarControls(cAvatarController, cAvatar.avatar);
	...
	var resolveControls = function() {
	    ...
	    myWorld.camera.position.x = cAvatar.avatar.getChildPosition('cameraTarget').x;
	    myWorld.camera.position.y = cAvatar.avatar.getChildPosition('cameraTarget').y;
	    myWorld.camera.position.z = cAvatar.avatar.getChildPosition('cameraTarget').z;
	    //myWorld.camera.position.z = cAvatar.avatar.position.z;
	    //console.log(cAvatar.avatar.getChildPosition('cameraActual'));
